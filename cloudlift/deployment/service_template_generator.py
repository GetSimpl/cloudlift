...
class ServiceTemplateGenerator(TemplateGenerator):
    ...
    def _add_service_alarms(self, svc, config):
        ...
        ecs_no_running_tasks_alarm = Alarm(
            ...
            AlarmDescription=self._enrich_alarm_description('Alarm if the task count goes to zero, denoting service is down', config),
            Namespace='AWS/ECS',
    ...
    def _add_scalable_target_alarms(self, service_name, ecs_svc, config):
        max_scalable_target_alarm = Alarm(
            ...
            AlarmActions=[Ref(self.notification_sns_arn)],
            OKActions=[Ref(self.notification_sns_arn)],
            AlarmDescription=self._enrich_alarm_description('Triggers if desired task count of a service is equal to max_capacity, review auto scaling configuration if this alarm triggers', config),
    ...
    def _enrich_alarm_description(self, msg, config):
        playbook_url = config.get('alarm_playbook_url', '')
        if playbook_url == '':
            return msg
        msg = msg.rstrip('. ')
        return f"{msg}. More details can be found from {playbook_url}"
    ...
    def create_target_group_alarms(self, target_group_name, target_group, alb_full_name, alb_config):
        unhealthy_alarm = Alarm(
            ...
            AlarmActions=[Ref(self.notification_sns_arn)],
            OKActions=[Ref(self.notification_sns_arn)],
            AlarmDescription=self._enrich_alarm_description('Triggers if any host is marked unhealthy', alb_config),
        ...
        high_5xx_alarm = Alarm(
            ...
            AlarmActions=[Ref(self.notification_sns_arn)],
            OKActions=[Ref(self.notification_sns_arn)],
            AlarmDescription=self._enrich_alarm_description('Triggers if target returns 5xx error code', alb_config),
            Namespace='AWS/ApplicationELB',
    ...
    def _get_latency_alarms(self, alb_config, alb_full_name, target_group, target_group_name):
        high_p95_latency_alarm = Alarm(
            ...
            AlarmActions=[Ref(self.notification_sns_arn)],
            OKActions=[Ref(self.notification_sns_arn)],
            AlarmDescription=self._enrich_alarm_description('Triggers if p95 latency of target group is higher than threshold', alb_config),
            Namespace='AWS/ApplicationELB',
        ...
        high_p99_latency_alarm = Alarm(
            ...
            AlarmActions=[Ref(self.notification_sns_arn)],
            OKActions=[Ref(self.notification_sns_arn)],
            AlarmDescription=self._enrich_alarm_description('Triggers if p99 latency of target group is higher than threshold', alb_config),
            Namespace='AWS/ApplicationELB',
    ...
    def _add_alb_alarms(self, service_name, alb, alb_config):
        ...
        for elb_5xx_error_code in elb_5XX_error_codes_to_monitor:
            ...
            self.template.add_resource(Alarm(
                ...
                AlarmActions=[Ref(self.notification_sns_arn)],
                OKActions=[Ref(self.notification_sns_arn)],
                AlarmDescription=self._enrich_alarm_description(f'Triggers if {elb_5xx_error_code} response originated from load balancer', alb_config),
                Namespace='AWS/ApplicationELB',
                Period=60,
                ComparisonOperator='GreaterThanOrEqualToThreshold',
                Statistic='Sum',
        ...
        rejected_connections_alarm = Alarm(
            ...
            AlarmActions=[Ref(self.notification_sns_arn)],
            OKActions=[Ref(self.notification_sns_arn)],
            AlarmDescription=self._enrich_alarm_description('Triggers if load balancer has rejected connections because the load balancer had reached its maximum number of connections.', alb_config),
            Namespace='AWS/ApplicationELB',
            Period=60,
            ComparisonOperator='GreaterThanOrEqualToThreshold',
            Statistic='Sum',
